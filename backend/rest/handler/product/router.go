...
func (h *Handler) RegisterRoute(mux *http.ServeMux, middlewareManager *middleware.Manager) {
	...
	mux.Handle(
		"POST /products",
		middlewareManager.With(http.HandlerFunc(h.CreateProductHandler), middleware.AuthenticationJWT),
	)
	...
	mux.Handle(
		"GET /products/{id}",
		middlewareManager.With(http.HandlerFunc(h.GetProductHandler)),
	)
